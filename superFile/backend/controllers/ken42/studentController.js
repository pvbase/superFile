...
async function addStudentStrcuture(dbConnection, inputData) {
  ...
  let programFeesModel = dbConnection.model(
    "defaultProgramPlanFees",
    defaultPlan
  );
  ...
  try {
    let findProgramPlan = await programPlanModel.findOne({
      hedaId: inputData.studentDetails.programPlanId,
    });
    let findProgramPlanFees = await programFeesModel.findOne({
      programPlanId: inputData.studentDetails.programPlanId,
    });
    let findFeeStructure = await feeStructureModel.findOne({
      displayName: "FS_2021-22_001",
    });
