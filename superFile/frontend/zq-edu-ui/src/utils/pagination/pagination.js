...
class PaginationUI extends Component {
    ...
    handleClick = (e, index) => {
        ...
        this.setState({ currentPage: index, }, () => {
            ...
            this.props.onPaginationApi(this.state.currentPage, this.state.postPerPage, this.state.allData)
        })
    }
    PaginationChange = (e) => {
    ...
    componentDidMount = () => {
        this.setState({ totalPages: this.props.totalPages, allData: this.props.allData, currentPage: this.props.currentPage, postPerPage: this.props.limit })
    }
    changePageNum = (e) => {
                ...
                <div className="pagination-wrapper pagination-right">
                    <div className="pagination-wrapper-content">
                        <div className="page-select-range">
                            ...
                            <p className="pageofText"><input type="number" className="choose-page" onChange={this.changePageNum} value={currentPage} /> of {this.props.totalPages}</p><span><ChevronRightIcon className={currentPage == this.props.totalPages ? "pagination-icon light " : "pagination-icon  "} onClick={e => (currentPage == this.props.totalPages) ? null : this.handleClick(e, currentPage + 1)} /></span>
                        </div>
                    </div>
                </div>
            </div>
